...
# # Does scaling help transformer for transitivity in graphs?
...
def number_of_edges(dag):
    num_edges = 0
    for v in dag:
        num_edges += len(v.children)
    return num_edges

def relabel_nodes(dag, node_map):
    new_dag = []
    new_map = {}
    for v in dag:
        new_node = Node(node_map[v].id)
        new_map[v] = new_node
        new_dag.append(new_node)
    for v in dag:
        for u in v.children:
            add_edge(new_dag, new_map[u], new_map[v])
    return new_dag
...
def copy(graph):
    ...
    for u in graph:
        for v in u.children:
            add_edge(new_graph, node_map[u], node_map[v])
    return new_graph

def generate_dag(total_nodes, number_of_chains):
    if number_of_chains > total_nodes:
        print("Not enough nodes")
        return
    
    k = int(total_nodes/number_of_chains)
    C = number_of_chains
...
def generate_train_test_pairs(dag,node_map,M_test):
    ...
    #adding non-reverse negative edges
    ...
    random.shuffle(train_positive_edges)
    random.shuffle(train_negative_edges)
    random.shuffle(test_positive_edges)
    random.shuffle(test_negative_edges)
...
parser.add_argument('--number_of_chains', type=int, default=10)
...
# # Execute Data Scaling Experiments

# In[ ]:


total_nodes_list = [50,100,200,400,800]
epochs = [15000,10000,10000,5000,5000]

for i in range(len(total_nodes_list)):
    
    
    total_nodes = args.total_nodes = total_nodes_list[i]
    num_epochs = args.num_epochs = epochs[i]
    ...
    for seed in seeds:
        ...
        print(f"Running with d_emb = {d_emb} and seed = {seed}")

        dag,node_map = generate_dag(total_nodes=args.total_nodes, number_of_chains= args.number_of_chains)
        ...
        flops_file = "flops_" + str(total_nodes) +  "_chains_" + str(number_of_chains) + "_d_model_" + str(d_model) + ".npy"

        train_loss_file = "seed_" +  str(seed) + "_train_loss_nodes_" + str(total_nodes) + "_chains_" + str(number_of_chains) + "_d_model_" + str(d_model) + ".npy"
        test_loss_file = "seed_" +  str(seed) + "_test_loss_nodes_" + str(total_nodes) + "_chains_" + str(number_of_chains) + "_d_model_" + str(d_model) + ".npy"

        train_acc_file = "seed_" +  str(seed) + "_train_acc_nodes_" + str(total_nodes) + "_chains_" + str(number_of_chains) +  "_d_model_" + str(d_model) + ".npy"
        test_acc_file = "seed_" +  str(seed) + "_test_acc_nodes_" + str(total_nodes) + "_chains_" + str(number_of_chains) +  "_d_model_" + str(d_model) + ".npy"

        flops = []

        N= 2* d_model*args.num_layers*(2*d_model+d_ff)
        ...
        while True:
            ...
            print("Epoch {}: Acc : Tr : {:0.6f} ({}),  Te: {:0.6f} ({}), Loss : Tr : {:0.6f} Te: {:0.6f}, LR: {:0.6f}".format(
                epoch+1,best_train_acc,best_train_epoch , best_test_acc,best_test_epoch , avg_train_loss, avg_test_loss, scheduler.get_last_lr()[0]))
